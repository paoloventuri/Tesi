...
\item \textit{from\_serializeble(dict)}, riceve un dizionario e da esso crea un'istanza della classe usando le informazioni contenute.
\end{itemize}

Per tali metodi inoltre deve valere la seguente proprietà: il risultato dell'applicazione di entrambi i metodi su di un oggetto porta alla creazione di un nuovo oggetto uguale a quello di partenza.
...
In questo modo si riesce a ricavare sia il building\_id che il floor\_id dal file DXF: questi dati vengono perciò passati al costruttore del Floor insieme alla lista di Room andando a creare così un oggetto contenente tutti i dati letti dalla piantina.
...
Il file CSV con le informazioni sulle aule didattiche della Divisione edilizia contiene:
\begin{itemize}
...
Dato che per il file SVG può essere definito uno stile grazie ad un CSS (Cascading Style Sheets) si è deciso di generarlo tramite un LESS.
