...
Nell'ultima parte dello stage invece ci si è concentrati sulla generazione di piantine interattive utili alla futura applicazione multipiattaforma che costituisce lo scopo finale del progetto Spazi-Unimi %%SAM: possiamo considerare la tesi uno stage?
...
%%SAM: scrivi molto bene Paolo, non ho niente da aggiungere perché sei stato molto chiaro.
...
\vspace{5mm} %5mm vertical space
\item i generatori, simili alle list comprehension che non occupano però memoria; %%SAM: dato che hai deciso citare, spiegare meglio cosa significa "non occupare memoria". Il concetto vero dietro ai generatori è quello di fornire la lazy evaluation: cioè, noi lo dichiariamo, e soltanto quando verrà effettivamente richiesta l'iterazione o l'accesso ai dati del generatore i sui elementi verranno effettivamente creati.
...
\vspace{5mm} %5mm vertical space
\item la parola chiave lambda, utilizzata per definire piccole funzioni utilizzate solo in certe zone del codice senza dover definire una funzione che non verrà più richiamata. %%SAM: anonymous function è il termine giusto. Viene dal lambda calculus che vale citare, comunque sempre uno dei concetti cardine della prog. funzionale
...
%%SAM: puoi sottolineare la facilità di trattare queste funzioni come oggetti. In tanti altri linguaggi per portare una funzione all'interno di una variabile ci vuole dei giri e malabarismi. (Rubi stesso, se vuoi mettere all'interno di una variabile un metodo di un oggetto, devi fare qualcosa come myvar = oggetto.methods("method_name"), mentre su python tutto è piu' semplice per il suo data model) Comunque sono cose in piu, no nc'è neanche bisogno di citare perché non è lo scopo della tua relazione. Solo se vuoi aumentare il brodo xD
...
Per quanto riguarda le performace Pyhton risulta migliore di altri linguaggi interpretati come PHP e Ruby e nonostante non sia paragonabile al C risulta abbastanza simile a linguaggi compilati (Java). %%SAM: ERRORE DI BATTITURA SU PYTHON (SCRITTO PYHTON). Comunque queste osservazioni non sono basate bene, e i linguaggi interpretati che hai scelto per comparare non sono opportuni. Ruby si, ma PHP è un altro discorso. Le performance di Ruby però sono del tutto paragonabile a python, migliore per qualche aspetti ma peggiore per altri. L'unico paragone possibile è tra python e C/Java, che avranno ovviamente performance migliore, ma essendo tipati staticamente hanno tante caratteristiche negative in comparazione.

Ultimo vantaggio dell'usare Python ma dall'elevata importanza è il fatto che possieda una vasta libreria standard ed esista uno svariato numero di librerie importabili che possono svolgere e implementare molte funzionalità e algoritmi. Inoltre è presente un framework di testing unitario (unittest) utile a testare i vari metodi presenti nelle classi implementate: ciò ha reso possibile l'applicazione di modelli di sviluppo software come il TDD (Test Driven Development). %%SAM: su tutti i linguaggi ormai esiste un framework del genere, forze non ha senso citare. Vale la pena invece citare che possiede "svariati framework utili e interessanti per la scrittura, analisi e esecuzione dei test" (anche se ormai ci siamo limitati a unittest, inizialmente tutto ciò ha pesato a favore).
...
%%SAM: Non è interamente vero, con mongo perdiamo un sacco di espressività per query, in speciale in termine di operazioni di JOIN annidate e complesse. Abbiamo anche lo svantaggio di ottenere come risultato sempre l'intero documento. Meglio non citare questa cosa.
...
%%SAM: suggerimento: "così come nei database relazionali più utilizzati", 
...
%%SAM: Per questi casi, in alcuni casi abbiamo risolto posteriormente utilizzando del mocking, in altri facendo il refactoring, in speciale nel caso degli algoritmi procedurali. Ci sono comunque stati casi in cui la testability era talmente bassa e la natura dei test era talmente umana/manuale che abbiamo deciso di non testarli.
...
%%SAM: suggerimento, qualcosa del genere: "come potete vedere l'utilizzo della libreria è estremamente facile, e gestisce tutta la complessità dei file DXF, rimappando il suo contenuto semantico in oggetti comodi da trattare"
...
\end{itemize}
%%SAM: potresti spiegare perchè abbiamo deciso di implementare queste classi e non abbiamo utilizzato una libreria esterna. (all'inizio avevamo bisogno di veramente poche funzionalità, l librerie disponibili tutte avevano una determinata dimensione)
...
\begin{itemize}
%%SAM: non esiste il concetto di "oggetto di tipo json". Json è un formato per rappresentare oggetti come STRING. Inoltre devi esplicitare che to_serializable è di istanza, from_serializable è classmethod (statico, è una factory sostanzialmente)
...
%%SAM: converte l'oggetto python complesso in un oggetto che può essere convertito in formato testuale json. Di solito abbiamo optato per rappresentare i nostri oggetti complessi utilizzando il tipo nativo "dizionari" di python.
...
%%SAM: Da un oggetto ricreato dalla sua propria rappresentazione json, ricostruisce l'oggetto python complesso.

\end{itemize}

%%SAM: In particolare, per i due metodi deve valere, a meno di cambiamenti di versioni, la seguente ugualianza:
% r = Drawable(<args>)
% r2 = Drawable.from_serializable( r.to_serializable() )
%% assertEqual(r, r2)  # must be always true!
...
In questo formato nella prima linea sono indicati i nomi delle colonne mentre in quelle successive sono inseriti i dati. Le varie colonne sono separate da un carattere specifico che viene deciso in fase di codifica: nel nostro caso si trattava di una virgola (','). %%SAM:è il caso più comune, che da pure il nome alla codifica: comma separated values (comma è la virgola)

Per la lettura di un file CSV in Python è bastato importare la libreria 'csv' 
%%SAM: già inclusa nella standard library di Python! Batteries included! :P
...
%%SAM: molto buona tutta questa parte!
\newpage
...
%%SAM: questa nota non è chiarissima, forse riscrivere la frase?
...
Il file CSV contente le informazioni sulle aule didattiche della Divisione edilizia contiene: %% SAM: occhio al typo: "contente". Riscrivere questa frase!
\begin{itemize}
...
\end{python}
%% SAM: questo è una dictionary comprehention. Forse conviene citarla come una variazione della list comprehention, di cui hai parlato?
\vspace{5mm} %5mm vertical space
...
%% SAM: manca il comando piu' importante: cow! xD
\end{itemize}
...
%% SAM: lo scopo non è di definire un ORM, ma un ODM! Se vuoi citare l'ORM devi esplicitare che prima pensavamo che l'orm fosse una cosa, ma invece quello che facevamo era ODM in realtà! Consiglierei di togliere le citazioni sull'ORM e parlare solo di ODM.
%% SAM: conviene giustificare perché non abbiamo utilizzato nessuna delle molte già esistenti implementazioni di ODM di python. Il motivo era sempre quello: avevamo bisogno di molto poche delle funzionalità, e allora abbiamo deciso di utilizzare un'implementazione più semplice fatta ad hoc. Tutte le librerie ODM disponibili sono dei veri framework per manipolazione del database, noi volevamo qualcosa di piu' lightweight.
...
%% SAM: questa ultima frase è molto generica e forse un po' fuorviante. Se ci pensi i documenti di mongo sono a tutti gli effetti degli oggetti (concetto di oggetto di javascript è esattamente quello di mongo). Quello che guadagniamo nell'uso di queste classi è una mappatura intuitiva fra oggetti Python e loro rappresentazione su un DB a documenti. Disaccoppiamento fra le due cose anche.
...
%% SAM: ODMModel fornisce un'utile stratto di astrazione: se mai decidessimo o fosse necessario rappresentare/salvare queste informazioni in altro modo, si potrebbe sostituire il nostro ODMModel con un ORMModel, ad esempio.
...
In questo modo però si vengono a salvare documenti separati per informazioni riguardanti lo stesso palazzo; ad esempio i dati tradotti dai file DXF avrebbero come identificativo il 'legacy' mentre quelli provenienti dalla Divisione sistemi informativi utilizzerebbero il nuovo standard. %% SAM: D grande in Divisione?
...
%% SAM: dato che hai pure fornito nomi delle variabili, non conveniva dare gia' del pseudocodice? Oppure non entrare nel dettaglio delle variabili e lasciare la descrizione piu' "panoramica"?
...
%% SAM: la descrizione fin qua è molto precisa. Consiglierei di far vedere la struttura del dizionario, con le chiavi, ecc, in modo da fornire un esempio all'utente. Per me è molto chiaro tutto ciò, ma mi chiedo se è per il fatto che conosco già il formato del documento del db.
...
Giunti a questo punto ci si è resi conto della necessità di svolgere alcune operazioni da svolgere sui dati andavano effettuate esattamente prima del loro salvataggio nel database (o subito dopo). %% SAM: prima frase: hai usato svolgere 2 volte in fila, sembra strano.
Per richiamare queste operazioni si è quindi deciso di implementare in ODMModel una politica di 'callback'. 

\newpage

Per callback si intende una funzione che viene passata come argomento ad un'altra, 
%% SAM: suggerimento: con lo scopo di essere richiamata posteriormente. Si usa quando l'azione desiderata non può essere eseguita subito ma dev'essere posticipata per qualche ragione. Nel nostro caso...
...
%% SAM: sugg: Gli aspetti estetici (colori, contorni, ecc) del file SVG possono essere definiti su un file di stili separato CSS (Cascading Style Sheets). Abbiamo utilizzato un \textit{superset} di CSS chiamato LESS. Si tratta di un preprocessore di CSS in quanto ...
...
%% SAM: La tua conclusione è eccezionale. Nient'altro da dire :) Complimenti (dottor) Paolo, e grazie ancora una volta!
