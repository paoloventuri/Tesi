...
\definecolor{yellow}{RGB}{255,120,0}
...
\newcommand\mergestyle{\lstset{
basicstyle=\ttfamily\fontsize{10pt}{10pt}\selectfont,
commentstyle=\color{black},
otherkeywords={building_number,l_b_id,coordinates,type,address,building_name,floors,f_id,walls,unidentified_,rooms,windows,cat_id,polygon,R054,R053,room_name,equipments,capacity,accessibility,"_id,dxf,edilizia,easyroom,merged},
keywordstyle=\color{green},
belowskip=2em
}}
...
Per quanto riguarda l'organizzazione è risultato molto utile e semplice da utilizzare Trello\footnote{\url{https://trello.com}}: si tratta di un'applicazione basata su cartelloni (board).
...
\item implementazione della funzionalità, tramite il test viene definito il codice e si controlla la sua correttezza;
...
\begin{itemize}
\item Polygon, definibile come una collezione di punti ordinata, tali punti una volta collegati formano un poligono;
...
In pratica passando un dizionario a \textit{from\_serializable} e applicando al risultato \textit{to\_serializable} si ottiene un dizionario uguale a quello iniziale (stessa cosa con l'ordine dei metodi invertito se si parte da un oggetto di `model').\\
...
Dopo aver letto il file con dxfgrabber come visto in precedenza viene richiamata una funzione (\textit{\_extract\_entities()}) che scorre la lista di entità lette e controlla se appartengono alla tipologia e ai layer di nostro interesse.
...
La procedura si occupa di ricavare dal DXF letto i dati del layer `CARTIGLIO' nel quale sono contenute le informazioni riguardanti ciò che è stato disegnato nella cartina (nome del palazzo, disegnatore, data. etc.) tra cui il nome del piano.
...
In questo modo nella maggior parte dei casi si riesce a ricavare sia il building\_id che il floor\_id dal file DXF: questi dati vengono perciò passati al costruttore del Floor insieme alla lista di Room andando a creare così un oggetto contenente tutti i dati letti dalla piantina.
...
Per rendere le informazioni lette meglio manipolabili in futuro sono state memorizzate in una lista di dizionari in cui le chiavi corrispondono ai nomi delle colonne lette (header).
...
Dato che il progetto Spazi-Unimi prevede di operare principalmente su dati edilizi i quali risultano statici a meno di rare variazioni (ad esempio un cambio di sede per un dipartimento, delle ristrutturazioni o dei cambi di destinazione delle stanze) si è deciso di implementare un sistema ETL che definisce un database di tipo Data Warehouse.
...
Il file CSV con le informazioni sulle aule didattiche della Divisione Manutenzione edilizia e impiantistica contiene:
\begin{itemize}
\item building\_id, corrispondente a uno di quelli nel file degli edifici, tale dato è sempre presente per ogni aula e risulta utilissimo per definire in che palazzo si trova tale stanza;
\item address, informazione ridondante rispetto al file precedente, è sempre presente per ogni aula;
\item legacy\_floor, identificativo proprio della Divisione Manutenzione edilizia e impiantistica per indicare in che piano si trova l'aula, l'informazione è sempre presente ma non è di immediata lettura e il livello di accuratezza si può verificare solo controllando stanza per stanza sulle piantine fornite;
...
\item buiding\_name, indica una denominazione specifica dell'edificio a cui la stanza appartiene, l'informazione risulta poco utile dal lato utente perché spesso contiene nomi utili alla Divisione Manutenzione edilizia e impiantistica, il dato è comunque presente nella maggior parte dei casi;
...
\item building\_name, il nome dell'edificio è sempre presente ma è in molti casi troppo generico rispetto alle informazioni fornite dalla Divisione Manutenzione edilizia e impiantistica;
...
\item per effettuare l'unione delle informazioni di un palazzo bisogna attendere che i dati di tutte le fonti siano presenti: ciò nel nostro caso può richiedere molto tempo (a causa della differenza di frequenza degli aggiornamenti per le varie fonti) quindi risulta utile memorizzare i dati separati per poi applicare su di essi il processo di merge.
\end{itemize}
...
In pratica in un edificio con informazioni provenienti dalle tutte e tre le fonti la chiave `dxf' viene utilizzata come base e passata alla funzione \textit{match\_and\_merge\_floors} con le informazioni della chiave `edilizia' (la seconda per priorità). 
Il risultato di tale merge viene poi passato alla stessa procedura con la chiave rimanente cioè `easyroom'.
...
Tali procedure vanno perciò richiamate ogni volta che si deve rieseguire l'unione di dati a seguito di una modifica degli stessi.
...
\begin{merge}[title=Esempio di documento salvato nel database , frame=single]
{
"_id" : "31110",
"dxf" : {...},
"easyroom" : {...},
...
"merged" : {
   ...
   "address" : "Via Saldini, 50, Milano, 20133",
   "building_name" : "Matematica",
   "floors" : [ 
      {
         "f_id" : "-05",
         "walls" : [...],
...
Il progetto Spazi-Unimi come detto si proponeva di creare una applicazione multi-piattaforma che aiutasse gli utenti ad orientarsi nel contesto dell'Università degli Studi di Milano.
...
Questa serie di problemi hanno così allungato molto il tempo di sviluppo della parte back-end non lasciando tempo per lo sviluppo dell'applicazione vera e propria.
...
Inoltre la maggior parte del codice è coperta da test e ciò ci ha portato ad avere una buona sicurezza nel considerare i risultati delle varie elaborazioni come positivi.\\
...
Tale problematica si evidenzia maggiormente in fase di definizione delle API e può essere risolta proprio definendo un'altra collection contenente le informazioni delle aule.\\
