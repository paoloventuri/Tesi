...
Altre volte come detto nell'analisi dei dati l'indirizzo non fornisce delle informazioni esatte sulla posizione del palazzo (ad esempio "LODI OSPEDALE VETERINATRIO Ed. 1").
Si è quindi deciso nella procedura sanitize\_building di analizzare tale valore di indirizzo in modo da uniformarlo quando lo si ritiene utile mentre in caso contrario viene segnalato il problema e il campo viene lasciato vuoto.
...
Per questo motivo si è deciso di implementare un algoritmo in grado di analizzare i dati presenti nel database e di fornire in output, sempre grazie al Logger, una serie di dati utili a comprendere lo stato delle fonti.
...
Tali informazioni risultano molto utili per analizzare i dati di un singolo palazzo ma essendoci 262 edifici memorizzati è dispendioso trarre informazioni più generali sulle fonti.
...
Il restante caso (da edilizia ad easyroom) presenta invece una percentuale del 63,1\% che testimonia la maggior quantità di informazioni in possesso alla Divisione Manutenzione edilizia e impiantistica.
...
Avendo salvato nella chiave 'merged' anche le informazioni inerenti ai poligoni che formano le stanze basta andare a ricreare le piantine in file di tipo immagine.

La scelta è ricaduta sul formato SVG (Scalable Vector Graphics) per alcuni motivi:
\begin{itemize}
...
Innanzitutto sono state esplorate le librerie disponibili alla generazione di file SVG in Python 3: la libreria selezionata è stata svgwrite.
...
Il nostro LESS ha quindi il compito di definire un colore per ogni gruppo di categorie i quali sono specificati nel dizionario in room\_categories. 
...
Queste categorie (ad esempio i depositi di materiale o le cabine elettriche) non portando una reale utilità sono state rese nei file SVG delle stanze con tipologia "Sconosciuto". 
Ciò è stato reso possibile all'attributo 'scope' presente sempre nel dizionario delle room\_categories: le categorie con scopo non didattico vengono perciò rimappate in una categoria a parte cioè "Sconosciuto".
...
In tale layer sono presenti esattamente i disegni delle porte come da progetto edilizio ma la loro analisi è risultata molto difficile a causa della non uniformità del modo in cui sono state disegnate.
...
La prima cosa che si può dire giunti alla fine del periodo di tirocinio previsto (18 settimane) è che il progetto è più o meno a metà strada per giungere alla sua conclusione.
...
Ben presto infatti ci si è accorti che le problematiche relative allo sviluppo e del sistema erano molto più complesse rispetto alle stime iniziali.
...
Ad esempio ciò che ci ha portato a definire un algoritmo di inferenza dei piani è stata la mancata uniformità nello standard di codifica dei nomi dei file DXF.
...
L'affidabilità deriva dall'aver cercato di ottimizzare sempre i dati a nostra disposizione: invece di segnalare le criticità e buttare le informazioni si è sempre cercato di analizzare quanto l'informazione fosse utilizzabile e migliorabile.
Il risultato sono stati gli algoritmi di controllo presenti in ogni parte del codice che validano o sistemano i dati fornendo un report utile agli utenti che li aggiornano.
...
Nonostante un team di più persone possa solitamente produrre codice scritto in maniera molto diversa passando da membro a membro si è cercato di adottare lo stile consigliato di Python\footnote{https://www.python.org/dev/peps/pep-0008}
 leggermente adattato per rispecchiare le nostre esigenze.
...
Sempre con l'obbiettivo della leggibilità del codice scritto si è inoltre aggiunto ad ogni funzione una docstring cioè un commento con il compito di descrizione.
...
\item la specifica del valore di ritorno della funzione;
\item se necessaria una descrizione più esaustiva e completa delle operazioni svolte in tale procedura. 
\end{itemize}

\vspace{5mm} %5mm vertical space

Per quanto riguarda invece le scelte tecnologiche effettuate all'inizio del progetto si possono trarre ora delle conclusioni vista la quantità di tempo passata utilizzandole.
Python 3 si dimostrato un linguaggio molto potente ed efficace, e grazie all'approccio multiparadigma ha permesso di implementare sempre il codice nella maniera più semplice ed intuitiva.
...
Altro punto a favore di MongoDB sono le query geospaziali già definite: creando oggetti GeoJson e un indice sui campi che li contengono si possono effettuare facilmente delle richieste per sapere dove ci si trova.
...
La collection interessata dai test conteneva sempre dati su 1500 edifici aventi ognuno 5 piani in cui vi erano sempre 10 stanze mappate: tale insieme è molto superiore a quello realmente a disposizione ma è servito per comprendere meglio i tempi di risposta del database.   
...
\item ricerca di una stanza con dati sulle aule annidati in quelli degli edifici e con indice creato sui room\_id, tempo di calcolo: 0.17 secondi;
...
Ciò ci porta però alla problematica principale emersa dall'utilizzo di MongoDB: effettuando una query in cui ad esempio si specifica un parametro di una stanza da cercare il database ritorna la lista di documenti rappresentanti gli edifici che contengono almeno un'aula che soddisfa il parametro.
...
Dal mio punto di vista lo sviluppo del progetto Spazi-Unimi ha portato una notevole crescita personale; il fatto di aver imparato partendo da zero nuove tecnologie (Python 3, MongoDB, etc...) oltre ad aumentare le mie competenze mi ha fatto comprendere appieno l'importanza di cercare un continuo miglioramento e aggiornamento nel campo dell'informatica.

Altra considerazione importante è che questo per me è stato il primo progetto svolto in team al di fuori dei laboratori didattici svolti durante le lezioni universitarie.
...
In molti casi non si è infatti riusciti a mantenere le linee guida dell'XP (Extreme Programming) soprattutto per quanto riguarda il principio KISS (Keep It Simple, Stupid).
Volendo utilizzare un approccio Big Design Up Front (BDUF) in molti casi infatti si è andati a complicare la progettazione e l'implementazione di certe parti del sistema.
...
A mio modesto parere però il prodotto sin qui sviluppato ha portato alla definizione di un database contente dati di qualità molto buona che utilizzati nella futura applicazione multipiattaforma porteranno un notevole beneficio per gli utenti dell'Università degli Studi di Milano.  
